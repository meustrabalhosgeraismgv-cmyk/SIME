...
const controller = require('../controllers/configuracaoMatriculaController');

async function verificarAcesso(req, res, next) {
  try {
    const { instituicaoId } = req.params;
    if (!['admin', 'instituicao'].includes(req.user.perfil)) {
      return res.status(403).json({ error: 'Apenas administradores e gestores podem gerir requisitos de matrícula' });
    }
...
router.post('/gerar', async (req, res, next) => {
  if (!['admin', 'instituicao'].includes(req.user.perfil)) {
    return res.status(403).json({ error: 'Apenas administradores e gestores podem gerar requisitos de matrícula' });
  }
  return next();
